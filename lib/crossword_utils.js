...
const isTile = type => tile => String(tile) === type;
export const isWhiteTile = isTile(WHITE_TILE);
export const isBlackTile = isTile(BLACK_TILE);
...
const symmetricalize = grid => {
  ...
  return R.splitEvery(15, symmetricalGridString.split(''))
    .map(row => row.join(''));
};
