...
const assignMiddleRow = grid => {
  const middleRowPattern = validMiddleRowPatterns[getRandomArbitrary(0, validMiddleRowPatterns.length)];
...
// TODO: to make this code testable we should extract the randomness from the function, to do this we could pass a function with the grid
// that returns a word pattern to be used. For the real application we could have a getRandomWordPattern(), but for test purposes
...
export function generateCrosswordPattern() {
  ...
  grid = transpose(R.addIndex(R.map)((column, columnIndex) => {
    // instead of always using a word pattern for the first column, we should assign word patterns according to the
    // middle row, so if the middle row has a 1 in the second column, then that column should have the word pattern.
    if (columnIndex % 2 === 0) {
      const middleRowIndex = parseInt(grid.length / 2, 10);
      const middleRowTile = grid[middleRowIndex][columnIndex];
      const allowedPatterns = R.filter(pattern => pattern[middleRowIndex] === middleRowTile, validWordPatterns);

      // do not change rows 1, 3, 5, 7, 8
      // possibly use three state variables, to indicate tiles that don't have any assignment (null)
